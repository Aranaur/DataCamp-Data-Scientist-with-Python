...
# %% Using merge_ordered()
# pd.merge_ordered(df1, df2, on='', left_on=, right_on=, how=, suffixes=(), fill_method='ffill')
...
bond_perc = ten_yr.melt('metric', var_name='date', value_name='close')

# Use query on bond_perc to select only the rows where metric=close
bond_perc_close = bond_perc.query('metric == "close"')

# Merge (ordered) dji and bond_perc_close on date with an inner join
dow_bond = dji.merge(bond_perc_close, on='date', how='inner', suffixes=('_dow', '_bond'))
